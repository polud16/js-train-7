...
function createSymbolWithDescription(property) {
  const symbol = Symbol(property); // Створюємо символ symbol з описом property
  return symbol.description; //Повертаємо опис символа symbol
}
...
function createSymbolProperty(property) {
  ...
  const obj = {};
  obj[sym] = property;
  return obj; // Створимо змінну sym яка дорівнює символу без опису
...
function createAndCompareTwoSymbols(value) {
  let a = Symbol("Test");
  let b = Symbol("Test");
  console.log(a === b); // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
}
...
function compareSymbolsForKey(symbol1, symbol2) {
  return symbol1 === symbol2; // Використовуємо оператор '===' для порівняння двох символів
...
function useSymbolToStringTag() {
  const myObject = {};
  const customTagSymbol = Symbol.toStringTag;
  myObject[customTagSymbol] = "CustomObject";
...
function useSymbolDescription() {
  ...
  return symbolDescription; // Створюємо символ mySymbol зі значенням "mySymbol".
...
function useSymbolIterator(obj) {
  ...
  return result; // Проходимо крізь елементи об'єкта obj, використовуючи цикл "for...of"
...
let myObject = {
  ...
  currentIndex: 0, // Додали властивість currentIndex та ініціювали її значенням 0

  [Symbol.iterator]() {
    return this; // Повертаємо сам об'єкт
  },

  next() {
    if (this.currentIndex < this.category.length) {
      const value = this.category[this.currentIndex].name;
      this.currentIndex++; // Збільшуємо індекс для наступної ітерації
